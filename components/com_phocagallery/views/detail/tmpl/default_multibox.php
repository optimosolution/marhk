...
if ($this->tmpl['mb_thumbs']) {
	...
	if (!empty($this->tmpl['mb_thumbs_data'])) {
		...
		foreach ($this->tmpl['mb_thumbs_data'] as $k => $v) {
			...
			echo '<div class="pg-multibox-thumbs-item"><a href="'.JRoute::_('index.php?option=com_phocagallery&view=detail&catid='. $v->catslug.'&id='.$v->slug.$tmplCom.'&Itemid='. JRequest::getVar('Itemid', 1, 'get', 'int')).'">';
			echo $img;
			echo '</a></div>';
